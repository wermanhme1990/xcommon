...
$$ENDIF //SOCKETS
$$IF(TOOLBAR_CUSTOMIZATION || CONTAINER || CONTAINER_SERVER || MINI_SERVER || FULL_SERVER || AUTOMATION)
...
$$IF(VERBOSE)
	...
	// if not yet present
$$ENDIF
$$IF(MOUSE_CUSTOMIZATION)
	InitMouseManager();
$$ENDIF
$$IF(IMMEDIATE_DOCKING)
	CBCGPDockManager::m_dockModeGlobal = DT_IMMEDIATE;
$$ENDIF
$$IF(CONTEXT_MENU_CUSTOMIZATION)
	InitContextMenuManager();
$$ENDIF
$$IF(KEYBOARD_CUSTOMIZATION)
	InitKeyboardManager();
...
$$ENDIF
$$IF(SKIN_MANAGER)
$$IF(DEFAULT_SKIN_FOLDER)
	InitSkinManager ();
$$ELSE
	InitSkinManager (_T("$$SKINS_FOLDER$$"));
$$ENDIF
$$IF(DOWNLOAD_SKINS)

	GetSkinManager ()->EnableSkinsDownload (_T("$$SKINS_URL$$"));
$$ENDIF	// DOWNLOAD_SKINS
...
void $$APP_CLASS$$::PreLoadState ()
{
$$IF(MOUSE_CUSTOMIZATION)

$$IF(PROJTYPE_MDI)
	GetMouseManager()->AddView (IDR_$$DOC$$TYPE, _T("My view"), IDR_$$DOC$$TYPE);
$$ELSE
...
$$IF(PROJTYPE_MDI)
BOOL $$APP_CLASS$$::SaveAllModified()
{
