		...
		))
	{
		...
		if (!pBar->IsFloating ())
		{
			...
			if (pBarClass->IsDerivedFrom (RUNTIME_CLASS (CBCGPMSMCategorieBar)))
			{
				...
				rt.bottom = rt.top + 1;
				dm.HighlightRect (rt, 15, -1, 0, RGB (255, 255, 255));

				rt.top++;
				rt.bottom++;
				dm.HighlightRect (rt, 40, -1, 0, RGB (255, 255, 255));

				rt.top++;
				rt.bottom = rectClient.bottom - 1;
				dm.HighlightRect (rt, 30, -1, 0, RGB (255, 255, 255));

				rt.top = rt.bottom;
				rt.bottom++;
				dm.HighlightRect (rt, 10, -1, 0, RGB (255, 255, 255));
			}
		}
	}
		...
		CBrush* pBrush, CBCGPToolbarButton* pButton)
{
	...
	else
	{
			...
			pMenuButton->GetParentWnd ()->IsKindOf (RUNTIME_CLASS (CBCGPMSMToolBar)))
		{
			...
			dm.HighlightRect (rect, 10, -1, 0, RGB (255, 255, 255));
		}
		else // other buttons
		{
