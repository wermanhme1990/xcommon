		...
		if (nCutTabsNum > 0)
		{
			...
			for (i = 0; i < m_iTabsNum; i ++)
			{
				...
				if (!m_bFlat)
				{
					...
					{
						m_pToolTip->SetToolRect (this, pTab->m_iTabID, 
							bIsTrucncated || pTab->m_bAlwaysShowToolTip || m_bCustomToolTips ? 
	...
	if (m_bScroll)
	{
		...
		m_btnScrollLeft.SetStdImage (
			...
			m_bIsOneNoteStyle || m_bIsVS2005Style || m_bFlat ? 
					CMenuImages::ImageBlack : CMenuImages::ImageDkGray,
...
BOOL CBCGPTabWnd::OnNotify (WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	...
	if (pNMHDR->code == TTN_SHOW && m_pToolTip->GetSafeHwnd () != NULL)
	{
		m_pToolTip->SetWindowPos (&wndTop, -1, -1, -1, -1,
			SWP_NOMOVE | SWP_NOACTIVATE | SWP_NOSIZE);
	}
...
void CBCGPTabWnd::UpdateScrollButtonsState ()
{
	...
	{
		m_btnScrollRight.SetStdImage (
			m_bHiddenDocuments ? CMenuImages::IdCustomizeArowDownBold : CMenuImages::IdArowDownLarge,
			m_bIsOneNoteStyle || m_bIsVS2005Style || m_bLeftRightRounded ? CMenuImages::ImageBlack : CMenuImages::ImageDkGray,
			CMenuImages::IdArowDownLarge);
	}
