...
void CBCGPControlBar::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if (!m_bCaptured && CanFloat ())
	{
		...
		if ((GetDockMode () & DT_IMMEDIATE) != 0 || pMiniFrame != NULL)
		{
			StoreRecentDockInfo ();
			if (pMiniFrame == NULL)
			{
		...
		else if ((GetDockMode () & DT_STANDARD) != 0)
		{
			EnterDragMode (TRUE);
		}
	}
...
void CBCGPControlBar::RemoveFromMiniframe (CWnd* pNewParent, BCGP_DOCK_METHOD dockMethod)
{
	...
	if (pMiniFrame != NULL)
	{
		...
		BOOL bDelayDestroy = ((dockMethod == DM_MOUSE /*|| dockMethod == DM_DBL_CLICK*/) && 
							  m_hwndMiniFrameToBeClosed == NULL);

		pMiniFrame->RemoveControlBar (this, FALSE, bDelayDestroy);
		if ((dockMethod == DM_MOUSE /*|| dockMethod == DM_DBL_CLICK*/) && 
			 m_hwndMiniFrameToBeClosed == NULL /*&& 
									   ...
									   bool bShow)
{
	...
	if (dockMethod == DM_MOUSE && (GetDockMode () & DT_IMMEDIATE) != 0)
	{
...
void CBCGPControlBar::OnStyleChanged(int nStyleType, LPSTYLESTRUCT lpStyleStruct)
{
	...
	if (nStyleType == GWL_EXSTYLE)
	{
		if (((lpStyleStruct->styleOld & WS_EX_LAYOUTRTL) != 0 && 
			...
			(lpStyleStruct->styleNew & WS_EX_LAYOUTRTL) != 0) && 
			GetParentDockBar () != NULL && IsHorizontal ())
		{
	...
	case idFloating:
		{
			...
			if (pTabbedBar != NULL)
			{
				...
				if (pTabWnd != NULL)
				{
					...
					{
						bWasFloated = pTabbedBar->FloatTab (pWnd, nTabID, DM_UNKNOWN);
						if (bWasFloated)
						{
