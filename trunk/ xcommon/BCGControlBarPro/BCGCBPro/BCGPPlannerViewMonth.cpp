		...
		for (int iDay = 0; iDay < 7; iDay++)
		{
			...
			if (m_nRowHeight <= rectDayCaption.Height () + 2)
			{
				...
				COLORREF clrText = OnFillPlannerCaption (
					pDC, rectDayCaption, bToday, bSelected, TRUE /* No border */);

				DrawCaptionText (pDC, rectDayCaption, strDate, clrText, DT_RIGHT,
					bToday && bSelected);
			}
...
void CBCGPPlannerViewMonth::SetDateInterval (const COleDateTime& date1, const COleDateTime& date2)
{
	...
	m_DateStart = GetFirstWeekDay2 (date1, CBCGPPlannerManagerCtrl::GetFirstDayOfWeek () + 1);
	m_DateEnd   = m_DateStart + COleDateTimeSpan (m_nDuration - 1, 23, 59, 59);
