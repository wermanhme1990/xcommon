					...
					!g_pUserToolsManager->InvokeTool (uiCmdId)))
				{
					...
					if (pParentMenu != NULL)
					{
						...
						if (pParentMenu->m_pParentRibbonElement != NULL)
						{
							...
							pParentMenu->m_pParentRibbonElement->SetDroppedDown (FALSE);
        ...
        else
        {
				...
				m_bPaletteMode)
			{
				...
				else
				{
					...
					item.m_bText = TRUE;
					item.m_bImage = FALSE;
					item.m_iUserImage = CMD_MGR.GetMenuUserImage (uiCmd);
...
void CBCGPPopupMenuBar::OnToolbarImageAndText() 
{
	...
	{
		CMD_MGR.EnableMenuItemImage (pButton->m_nID, TRUE, iImage);
	}
