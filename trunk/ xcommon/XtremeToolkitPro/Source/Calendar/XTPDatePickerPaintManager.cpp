...
void CXTPDatePickerPaintManager::RefreshMetrics()
{
	...
	{
		m_clrSelectedDayBack = m_clrBtnFace;
		m_clrSelectedDayText = RGB(255, 255, 255);
		m_clrHighlightToday = RGB(130, 0, 0);
	}
}

XTPCalendarTheme CXTPDatePickerPaintManager::GetPaintTheme() const
{
...
{
	ASSERT(ePaintTheme <= xtpCalendarThemeOffice2003);
...
void CXTPDatePickerPaintManager::DrawScrollTriangle(CDC* pDC, CRect rcSpot, BOOL bLeftDirection, COLORREF clrColor)
{
	CPoint ptCenter(rcSpot.CenterPoint());

	if (bLeftDirection)
		Triangle(pDC, ptCenter, CPoint(ptCenter.x + 4, ptCenter.y - 4), CPoint(ptCenter.x + 4, ptCenter.y + 4), clrColor);
	else
		Triangle(pDC, ptCenter, CPoint(ptCenter.x - 4, ptCenter.y - 4), CPoint(ptCenter.x - 4, ptCenter.y + 4), clrColor);
...
void CXTPDatePickerPaintManager::DrawMonthHeader(CDC* pDC, CXTPDatePickerItemMonth* pMonth)
{
	...
	if (GetPaintTheme() >= xtpCalendarThemeOffice2003)
	{
		pDC->FillSolidRect(rcHeaderBk, GetHeaderBackColor());
	}
	else
	{
		int nVertDiv = 0;
...
CXTPDatePickerTheme::~CXTPDatePickerTheme()
{
	m_Sink.UnadviseAll();
}

void CXTPDatePickerTheme::SetControl(CXTPDatePickerControl*  pControl)
{
	m_Sink.UnadviseAll();

	TBase::SetControl(pControl);

	if (pControl && XTPOffice2007Images() && XTPOffice2007Images()->GetConnection())
	{
		m_Sink.Advise(XTPOffice2007Images()->GetConnection(), XTP_NC_COMMON_OFFICE2007IMAGES_CHANGED,
					   &CXTPDatePickerTheme::OnEvent_Office2007ImagesChanged);
	}
}

void CXTPDatePickerTheme::OnEvent_Office2007ImagesChanged(XTP_NOTIFY_CODE Event, WPARAM wParam, LPARAM lParam)
{
...
void CXTPDatePickerTheme::CTOMonthHeader::RefreshMetrics()
{
	...
	m_clrButtons.SetStandardValue(GetSysColor(COLOR_WINDOWTEXT));
...
void CXTPDatePickerTheme::DrawScrollTriangle(CDC* pDC, CRect rcSpot, BOOL bLeftDirection, COLORREF clrColor)
{
	// TODO: customize L/R buttons
	TBase::DrawScrollTriangle(pDC, rcSpot, bLeftDirection, clrColor);
}
...
{
	// TODO: customize
	TBase::DrawButton(pDC, rcButton, _T(""), bIsDown, bIsHighLight);
...
void CXTPDatePickerTheme::CTOMonthHeader::Draw(CDC* pDC, CXTPDatePickerItemMonth* pMonth)
{
	...
	if (!rcLeftScroll.IsRectEmpty())
	{
		...
		// draw left triangle itself
		if (pMonth->GetShowLeftScroll())
			m_pTheme->DrawScrollTriangle(pDC, rcLeftScroll, TRUE, m_clrButtons);
	}
	...
	if (!rcRightScroll.IsRectEmpty())
	{
		...
		// draw right triangle itself
		if (pMonth->GetShowRightScroll())
			m_pTheme->DrawScrollTriangle(pDC, rcRightScroll, FALSE, m_clrButtons);
	}
}
...
void CXTPDatePickerThemeOffice2007::RefreshMetrics()
{
	...
	m_clrLineTop.SetStandardValue(GetColor2(_T("Lines"), RGB(154, 198,255)));
	m_clrLineLeft.SetStandardValue(m_clrLineTop.GetStandardColor());
}

void CXTPDatePickerThemeOffice2007::SetPaintTheme(XTPCalendarTheme ePaintTheme)
{
	ASSERT(ePaintTheme == xtpCalendarThemeOffice2007);
...
void CXTPDatePickerThemeOffice2007::CTOMonthHeader::RefreshMetrics()
{
	...
	m_clrBackground.SetStandardValue(m_pTheme->GetColor2(_T("MonthHeaderBk"), RGB(191, 219, 255)));
	m_clrButtons.SetStandardValue(m_pTheme->GetColor2(_T("MonthHeaderScrollBattons"), RGB(111, 157, 217)));
}

COLORREF CXTPDatePickerTheme::GetColor2(LPCTSTR pcszColorName, COLORREF clrDefault)
{
	if (this == NULL)
		return clrDefault;
	...
	{
		clrColor = XTPOffice2007Images()->GetImageColor(_T("DatePicker"), pcszColorName);
	}
