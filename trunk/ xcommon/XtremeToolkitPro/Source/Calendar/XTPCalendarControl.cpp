...
void CXTPCalendarControl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (IsToolTipVisible())
	{
		...
		HideToolTip();
		bStartEditing = m_wndTip.IsAdvancedMode() && rcTip.PtInRect(point);
	}
...
void CXTPCalendarControl::SwitchActiveView(XTPCalendarViewType eViewType)
{
	...
	else if (eViewType == xtpCalendarFullWeekView)
	{
		SetActiveView(GetDayView());
...
void CXTPCalendarControl::ShowToolTip(const CString& strText, const CRect rcToolTip,
									  CFont* pFont, BOOL bAdvanced)
{
...
void CXTPCalendarControl::SetPaintTheme(XTPCalendarTheme ePaintTheme)
{
	...
	if (ePaintTheme == xtpCalendarThemeOffice2007)
	{
		if (!m_pTheme)
		{
