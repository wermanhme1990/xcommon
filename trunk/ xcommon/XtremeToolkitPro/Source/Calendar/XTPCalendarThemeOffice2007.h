...
public:
	...
	DECLARE_THEMEPART(CTOPrevNextEventController, CXTPCalendarThemePart)
		...
		CTOPrevNextEventController()
		{
			m_nRefreshTimerID = 0;
			m_nRefreshDaysPerIdleStep = 2;
			m_nIdleStepTime_ms = 120;
			...
			m_ndtMinScanDay = (long)COleDateTime::GetCurrentTime();
			m_ndtMaxScanDay = m_ndtMinScanDay + 365 * 1;
			m_ndtMinScanDay -= 365 * 1;
		}

		virtual ~CTOPrevNextEventController() {
	...
	protected:
		...
		int          m_nRefreshDaysPerIdleStep;
		int          m_nIdleStepTime_ms;
		...
		class CDaysArray : public CXTPArrayT<long, long>
		{
		public:
			int AddDay(long nDay)
			{
				int nCount =  GetSize();
		...
		CDaysArray m_arNotEmptyDays;
	};
	friend class CTOPrevNextEventController;
	...
	virtual CXTPOffice2007Image* GetBitmap(LPCTSTR pcszStdBmpName);
	...
	DECLARE_THEMEPART_MEMBER_(7, CTOPrevNextEventController, PrevNextEventController, CXTPCalendarThemePart)

//  DECLARE_THEMEPART_MEMBER_(8, CTOPrevNextEventButton, PrevEventButton, CXTPCalendarThemePart)
//  DECLARE_THEMEPART_MEMBER_(9, CTOPrevNextEventButton, NextEventButton, CXTPCalendarThemePart)
