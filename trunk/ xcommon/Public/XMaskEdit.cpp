...
IMPLEMENT_DYNAMIC(CXMaskEdit, CXFlatEdit)

BEGIN_MESSAGE_MAP(CXMaskEdit, CXFlatEdit)
	//{{AFX_MSG_MAP(CXMaskEdit)
	ON_WM_CHAR()
	ON_WM_KEYDOWN()
	ON_WM_LBUTTONUP()
	ON_WM_CREATE()
...
void CXMaskEdit::EnableMask(LPCTSTR lpszMask, LPCTSTR lpszInputTemplate, 
...
void CXMaskEdit::DisableMask()
{
	m_strMask.Empty();
	m_strInputTemplate.Empty();
}

void CXMaskEdit::SetValidChars(LPCTSTR lpszValid)
{
	if (lpszValid != NULL)
	{
		m_strValid = lpszValid;
...
BOOL CXMaskEdit::IsMaskedChar(TCHAR chChar, TCHAR chMaskChar) const
{
	// ------------------------------
...
BOOL CXMaskEdit::SetValue(LPCTSTR lpszString, BOOL bWithDelimiters)
{
...
const CString CXMaskEdit::GetMaskedValue(BOOL bWithSpaces) const
{
	ASSERT(m_strMask.IsEmpty() == m_strInputTemplate.IsEmpty());
	ASSERT(m_strMask.GetLength() == m_strInputTemplate.GetLength());
...
void CXMaskEdit::SetWindowText(LPCTSTR lpszString)
{
...
void CXMaskEdit::GetWindowText(CString& strString) const
{
...
int CXMaskEdit::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
void CXMaskEdit::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	// --------------------------------------
...
void CXMaskEdit::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if (m_bSelectByGroup)
	{
		// -----------------
...
BOOL CXMaskEdit::CheckChar(TCHAR chChar, int nPos) // returns TRUE if the symbol is valid
{	
	ASSERT(m_strMask.IsEmpty() == m_strInputTemplate.IsEmpty());
	ASSERT(m_strMask.GetLength() == m_strInputTemplate.GetLength());
...
void CXMaskEdit::OnCharPrintchar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	ASSERT(m_strMask.IsEmpty() == m_strInputTemplate.IsEmpty());
	ASSERT(m_strMask.GetLength() == m_strInputTemplate.GetLength());
...
void CXMaskEdit::OnCharBackspace(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	ASSERT(m_strMask.IsEmpty() == m_strInputTemplate.IsEmpty());
	ASSERT(m_strMask.GetLength() == m_strInputTemplate.GetLength());
...
void CXMaskEdit::OnCharDelete(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	ASSERT(m_strMask.IsEmpty() == m_strInputTemplate.IsEmpty());
	ASSERT(m_strMask.GetLength() == m_strInputTemplate.GetLength());
...
void CXMaskEdit::GetGroupBounds(int &nBegin, int &nEnd, int nStartPos, BOOL bForward)
{
	ASSERT(m_strMask.IsEmpty() == m_strInputTemplate.IsEmpty());
	ASSERT(m_strMask.GetLength() == m_strInputTemplate.GetLength());
...
LRESULT CXMaskEdit::OnPaste (WPARAM, LPARAM)
{
...
void CXMaskEdit::EnableGetMaskedCharsOnly( BOOL bEnable /*= TRUE*/ )
{
	m_bGetMaskedCharsOnly = bEnable;
}
void CXMaskEdit::EnableSetMaskedCharsOnly( BOOL bEnable /*= TRUE*/ )
{
	m_bSetMaskedCharsOnly = bEnable;
}
void CXMaskEdit::EnableSelectByGroup( BOOL bEnable /*= TRUE*/ )
{
	m_bSelectByGroup = bEnable;
}