...
CSize CBCGPMSMCaptionBar::CalcFixedLayout (BOOL, BOOL bHorz)
{
	ASSERT_VALID(this);
...
LRESULT CBCGPMSMCaptionBar::OnSetText (WPARAM, LPARAM lParam)
{
	TRY
	{
		...
		else
		{
			...
			_tcscpy(m_strCaption.GetBufferSetLength((int)_tcslen((LPCTSTR)lParam)),
				(LPCTSTR)lParam);
