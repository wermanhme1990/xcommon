...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_pWndToolBoxBar->AttachToTabWnd (pTabbedBar, DM_SHOW, TRUE, &pTabbedBar);
...
void CMainFrame::SetDockingBarsIcons (BOOL bHiColorIcons)
{
	...
	HICON hDynamicHelpIcon = (HICON) ::LoadImage (::AfxGetResourceHandle (), 
				MAKEINTRESOURCE (bHiColorIcons ? IDI_DYNAMICHELP_VIEW_HC : IDI_DYNAMICHELP_VIEW),
				IMAGE_ICON, ::GetSystemMetrics (SM_CXSMICON), ::GetSystemMetrics (SM_CYSMICON), 0);
	...
	HICON hPropertiesBarIcon = (HICON) ::LoadImage (::AfxGetResourceHandle (), 
				MAKEINTRESOURCE (bHiColorIcons ? IDI_PROP_BAR_HC : IDI_PROPERTIES_BAR),
				IMAGE_ICON, ::GetSystemMetrics (SM_CXSMICON), ::GetSystemMetrics (SM_CYSMICON), 0);
							   ...
							   BOOL bIsToolBoxEx)
{
	...
	CBCGPToolBar::AddToolBarForImageCollection (IDR_MENU_IMAGES, 
		theApp.m_bHiColorIcons ? IDB_MENUIMAGES24 : 0);
		...
		pWndChild != NULL; pWndChild = pWndChild->GetNextWindow ())
	{
		...
		if (pMDIChild != NULL)
		{
			CBCGCBDotNetExampleView* pView = DYNAMIC_DOWNCAST (
				CBCGCBDotNetExampleView, pMDIChild->GetActiveView ());
			if (pView != NULL)
			{
