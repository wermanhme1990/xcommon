...
void CUndoBar::DoUndo ()
{
	...
	if (pChildFrame)
	{
		...
		if (pView &&
			pView->IsKindOf(RUNTIME_CLASS(CBCGCBDotNetExampleView)))
		{
			((CBCGCBDotNetExampleView*)pView)->DoUndo(pUndoButton);
		}
		else
		{
			ASSERT(FALSE);
...
CBCGPPopupMenu* CUndoButton::CreatePopupMenu ()
{
	...
	if (pChildFrame)
	{
		CBCGCBDotNetExampleView* pView = (CBCGCBDotNetExampleView*)pChildFrame->GetActiveView();
