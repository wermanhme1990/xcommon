...
public:
	// Group constructor
	CBCGPGridRow (const CString& strGroupName, DWORD_PTR dwData = 0);

	// Simple item
	CBCGPGridRow (int nColumnsNum = 0, DWORD_PTR dwData = 0);
...
//	virtual BOOL EditItem (CBCGPGridRow* pItem, LPPOINT lptClick = NULL);
	...
	virtual CString GetNameTooltip () { return _T("");} // TODO
...
public:
	...
	int HitTestColumn (CPoint point, BOOL bDelimiter = FALSE, int nDelta = 0) const;
	...
	BOOL GetGroupingColumnOrderArray (LPINT piArray, int iCount = -1) const;
...
{
	CBCGPGridPageInfo () : m_nFirstItem (0), m_nLastItem (0), m_lpData (NULL),
	...
	void Init (int nFirstItem, int nLastItem)
	{
...
protected:
	...
	CScrollBar	m_wndScrollHorz;		// Horizontal scroll bar
...
public:
	...
	BOOL GetColumnOrderArray (LPINT piArray, int iCount) const;
	BOOL SetColumnOrderArray (int iCount, LPINT piArray);
