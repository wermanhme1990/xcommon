...
HFONT CBCGPPlannerView::SetCurrFont (CDC* pDC)
{
	ASSERT_VALID (pDC);
...
void CBCGPPlannerView::AdjustLayout (BOOL bRedraw)
{
	...
	m_nRowHeight    = max ( max (tm.tmHeight + 4, m_pPlanner->GetClockIconSize ().cy + 2), 
	...
	m_pPlanner->GetScrollBarCtrl (SB_VERT)->SetWindowPos (NULL, m_rectApps.right, 
	...
	const COleDateTime& day, COLORREF clrText, int nAlign, BOOL bHighlight)
{
	...
	DrawCaptionText (pDC, rect, strDate, clrText, nAlign, bHighlight);
}
...
void CBCGPPlannerView::DrawAppointment (CDC* pDC, CBCGPAppointment* pApp, CBCGPAppointmentDrawStructEx* pDS)
{
	...
	if ((IsDrawTimeAsIcons () || (bAlternative && !pApp->m_bAllDay)) && 
		szClock != CSize (0, 0))
	{
		if (bAlternative)
		{
...
{
	return IsCaptureDatesMatched () && IsCaptureAreasMatched ();
}
