...
static const int xTabMargin = 15;
static const int yTabMargin = 4;

static const int yCaptionMargin = 8;

static const int xMargin = 2;
static const int yMargin = 4;
...
void CBCGPRibbonCmdUI::SetText (LPCTSTR lpszText)
{
	...
	if (m_pUpdated->GetText () != lpszText)
	{
...
void CBCGPRibbonMainButton::OnLButtonDown (CPoint point)
{
	...
	pMenu->Create (m_pRibbonBar, rectBtn.left - 2, rectBtn.CenterPoint ().y, (HMENU) NULL);
...
CSize CBCGPRibbonBar::CalcFixedLayout(BOOL, BOOL /*bHorz*/)
{
	...
	m_nCaptionHeight = 
		m_bReplaceFrameCaption ? tm.tmHeight + 2 * yCaptionMargin : 0;
	...
	else
	{
		...
		{
			m_nCaptionHeight = max (m_nCaptionHeight, sizeAQToolbar.cy);
		}
...
void CBCGPRibbonBar::OnSize(UINT nType, int cx, int cy) 
{
	...
	if (m_bRecalcCategoryWidth)
	{
		...
		for (int i = 0; i < m_arCategories.GetSize (); i++)
		{
			...
			m_nCategoryMinWidth = max (m_nCategoryMinWidth, pCategory->GetMinWidth (&dc));
		}

		dc.SelectObject (pOldFont);
		
		m_bRecalcCategoryWidth = FALSE;
	}

	const BOOL bHideAll = cx < m_nCategoryMinWidth;
		...
		int			nInsertAt)
{
	...
	CBCGPRibbonCategory* pCategory = new CBCGPRibbonCategory (
		this, lpszName, uiSmallImagesResID, uiLargeImagesResID,
		sizeSmallImage, sizeLargeImage);
...
void CBCGPRibbonBar::RecalcLayout ()
{
	...
	if (m_pPrintPreviewCategory == NULL)
	{
	...
	else
	{
		...
		int x = m_rectCaption.right - xMargin;
		...
		if (globalData.DwmIsCompositionEnabled ())
		{
			...
			{
				rect.top += cyFrameBorder;
				m_rectCaption.OffsetRect (0, cyFrameBorder);
			}
			...
			int nSysButtonsWidth = 4 * ncm.iCaptionWidth;
		...
		else
		{
			...
			const int yOffsetCaptionButton = max (0, 
				(m_rectCaption.Height () - sizeCaptionButton.cy) / 2 - 1);
	...
	if (m_pMainButton != NULL)
	{
		...
		else
		{
			m_pMainButton->SetRect (CRect (
				CPoint (rect.left, rect.top + yMargin), m_sizeMainButton));
	...
	if (!bHideAll)
	{
		...
		if (nTabs > 0)
		{
			const int nTabLeftOffset = m_sizeMainButton.cx + xMargin;
			const int nMaxTabWidth = 
				(rect.Width () - nTabLeftOffset - sizeTabElemens.cx - xMargin) / nTabs;
			...
			for (i = 0; i < m_arCategories.GetSize (); i++)
			{
				...
				int nTabWidth = rectTabText.Width () + 2 * xTabMargin;

				if (nTabWidth > nMaxTabWidth)
				{
				...
				if (uiContextID != 0 && m_bReplaceFrameCaption)
				{
					...
					if (pCaption != NULL)
					{
						...
						int nCaptionWidth = 
							max (nTabWidth, dc.GetTextExtent (pCaption->m_strText).cx + 2 * xTabMargin);
						...
						if (bIsFirstContextTab)
						{
							...
							{
								m_rectCaptionText.right = min (m_rectCaptionText.right, x);
							}
												 ...
												 BOOL bCheckPanelCaption)
{
	...
	if (bCheckActiveCategory && m_pActiveCategory != NULL && 
									...
									const CRect& rectImage)
{
	...
	CBCGPBaseRibbonElement* pElem = FindByID (uiCmdID, FALSE);
	if (pElem == NULL)
	{
		return FALSE;
	}
	...
	int x, int y, CBCGPBaseRibbonElement* pHit)
{
...

	{
		...
		else if (pHit != NULL)
		{
			...
			if (pHit->CanBeAddedToQAT () &&
				(m_QAToolbar.FindByID (nID) == NULL || nID == 0 || nID == (UINT)-1))
			{
				strItem.LoadString (IDS_BCGBARRES_ADD_TO_QAT);
				menu.AppendMenu (MF_STRING, idAddToQAT, strItem);
			}
			else if (pHit->m_bQuickAccessMode)
			{
	...
	case idCustomize:
		{
			...
			{
				CBCGPRibbonCustomize dlg (pParentFrame, this);
				dlg.DoModal ();
			}
...
//**************************************************************************
void CBCGPRibbonBar::ForceRelalcLayout ()
{
	m_bRecalcCategoryHeight = TRUE;
	m_bRecalcCategoryWidth = TRUE;
...
LRESULT CBCGPRibbonBar::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	...
	if (message == WM_NCLBUTTONUP)
	{
		...
		LRESULT lResult = globalData.DwmDefWindowProc (
			GetParent ()->GetSafeHwnd (), WM_NCHITTEST, wParam, lParam);
		...
		case HTMAXBUTTON:
			uiID = GetParent ()->IsZoomed () ? SC_RESTORE : SC_MAXIMIZE;
			break;
		...
		if (uiID != 0)
		{
			GetParent ()->PostMessage (WM_SYSCOMMAND, uiID);
			return 0;
		}
	}
