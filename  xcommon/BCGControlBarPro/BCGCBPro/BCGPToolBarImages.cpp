...
BOOL CBCGPToolBarImages::MirrorBitmap (HBITMAP& hbmp, int cxImage)
{
	...
	if (bmp.bmBitsPixel == 32)
	{
		...
		for (int iImage = 0; iImage < iCount; iImage++)
		{
			for (int y = 0; y < cy; y++)
			{
				DWORD* pRow1 = pBits + cx * (cy - y - 1) + iImage * cxImage;
                DWORD* pRow2 = pRow1 + cx - 1;

				for (int x = 0; x < cxImage / 2; x++)
				{
					DWORD* pColorLeft = pRow1;
					DWORD colorSaved = *pColorLeft;

					DWORD* pColorRight = pRow2;
