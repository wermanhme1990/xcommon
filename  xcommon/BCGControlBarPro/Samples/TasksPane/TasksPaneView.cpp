// TasksPaneView.cpp : implementation of the CTasksPaneView class
//
...
void CTasksPaneView::OnInitialUpdate()
{
	...
	GetDlgItem (IDC_CHECK_HICOLORIMAGES)->EnableWindow (m_bNavToolbar);

	CWindowDC dc (NULL);
	int nBitsPerPixel = dc.GetDeviceCaps (BITSPIXEL);
	GetDlgItem (IDC_CHECK_HICOLORIMAGES)->EnableWindow (nBitsPerPixel > 16);
...
void CTasksPaneView::OnMargins() 
{
	...
	if (m_bCustomizeMargins)
	{
		...
		m_nVMargin = CBCGPVisualManager::GetInstance ()->GetTasksPaneVertMargin ();
		m_nHMargin = CBCGPVisualManager::GetInstance ()->GetTasksPaneHorzMargin ();
		m_nGroupSpacing = CBCGPVisualManager::GetInstance ()->GetTasksPaneGroupVertOffset ();
		m_nTaskSpacing = CBCGPVisualManager::GetInstance ()->GetTasksPaneIconVertOffset ();
		m_nCaptionHeight = CBCGPVisualManager::GetInstance ()->GetTasksPaneGroupCaptionHeight();
		m_nTaskOffset = CBCGPVisualManager::GetInstance ()->GetTasksPaneTaskHorzOffset();
		m_nIconOffset = CBCGPVisualManager::GetInstance ()->GetTasksPaneIconHorzOffset();
...
void CTasksPaneView::OnCaptionicon() 
{
	...
	int nPage1Gr3 = pTaskPane->AddGroup (0, _T("Details"));
