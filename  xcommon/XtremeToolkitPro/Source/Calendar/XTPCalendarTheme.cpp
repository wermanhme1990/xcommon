...
CXTPOffice2007Image* CXTPCalendarTheme::GetBitmap(LPCTSTR pcszStdBmpName)
{
...
BOOL CXTPCalendarTheme::_LoadStdBitmap(int nBmpID, LPCTSTR pcszStdBmpName)
{
	...
	CXTPOffice2007Image* pBmp = GetBitmap(pcszStdBmpName);
	ASSERT(pBmp);
...
//#ifdef _DEBUG
//  int nTmp;
//  ASSERT(m_mapHSLShift.Lookup(eCLR, nTmp) == FALSE);
//#endif

//  m_mapScale[eCLR] = nScale;
}

//void CXTPCalendarTheme::SetColorEx(int eCLR, COLORREF clrColor)
//{
//  m_mapHSLShift[eCLR] = nScale;
//}
//
//void CXTPCalendarTheme::CrearColorsEx()
//{
//}


//void CXTPCalendarTheme::CTOColorsSet::SetColorHSL(int eCLR, int nLshift)
//{
//#ifdef _DEBUG
//  int nTmp;
//  ASSERT(m_mapScale.Lookup(eCLR, nTmp) == FALSE);
//#endif
//
//  m_mapHSLShift[eCLR] = nScale;
//}
...
COLORREF CXTPCalendarTheme::CTOColorsSet::GetColor(int nColorID, COLORREF clrBase)
{
	...
	//BOOL bHSL;
	int nScale = GetScale(nColorID);//, bHSL);

//  if (bHSL)
//  {
//      double dH, dS, dL;
//
//      XTPDrawHelpers()->RGBtoHSL(clrBase, dH, dS, dL);
//      dL += nScale;
//      COLORREF clrColor = XTPDrawHelpers()->HSLtoRGB(dH, dS, dL);
//
//      return clrColor;
//  }
...
//  rbHSL = m_mapHSLShift.Lookup(eCLR, nScale);
//  if (rbHSL)
//      return nScale;
...
void CXTPCalendarTheme::CTODayViewDay::Draw(CCmdTarget* pObject, CDC* pDC)
{
	...
	CXTPPropsStateContext autoStateCntAllDayPart(GetGroupPart()->GetAllDayEventsPart());
	...
	for (int i = 0; i < nGroupsCount; i++)
	{
		...
		if (pViewGroup)
		{
			...
			autoStateCntAllDayPart.RestoreState();

			autoStateCntGroup.RestoreState();
		}
	}
	autoStateCntAllDayPart.Clear();
...
/*  // dbg. calc bitmap layers
//
	CXTPOffice2007Image* pBmpHeader = m_pImagesStd->LoadFile(_T("CalendarExpandDay_xxx"));
	ASSERT(pBmpHeader);
	if (pBmpHeader)
	{
