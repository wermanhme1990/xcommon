...
BOOL AFX_CDECL CXTPCalendarMAPIDataProvider::GetParameterValueFromConStr(LPCTSTR lpszConnectionString, LPCTSTR pcszParameterName, CString& rstrValue)
{
	...
	if (strParam[0] == _T('\'') || strParam[0] == _T('"'))
	{
		...
		int nQuot2Idx = FIND_S(strParam, chQuot, 0);

		if (nQuot2Idx >= 0)
		{
			DELETE_S(strParam, nQuot2Idx, strParam.GetLength() - nQuot2Idx);
		}
	}
	else
	{
		int nParamEndIdx = FIND_S(strParam, _T(';'), 0);
		if (nParamEndIdx >= 0)
		{
			DELETE_S(strParam, nParamEndIdx, strParam.GetLength() - nParamEndIdx);
		}
	}
