...
BOOL CXTPCalendarView::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (pThemeX)
	{
		XTP_SAFE_CALL1(pThemeX->GetPrevNextEventControllerPartX(), OnLButtonDown(this, nFlags, point));
	}
...
void CXTPCalendarView::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (pThemeX)
	{
		XTP_SAFE_CALL1(pThemeX->GetPrevNextEventControllerPartX(), OnMouseMove(this, nFlags, point));
	}
	//---------------------------------------------------------------------------
...
CRect CXTPCalendarView::GetTooltipRect(const CPoint& ptHit, const XTP_CALENDAR_HITTESTINFO& hitInfo)
{
	CRect rcTip;
	rcTip.SetRectEmpty();
	...
	if (hitInfo.pViewGroup)
	{
		...
		if (pThemeX)
		{
			CRect rcExclude = XTP_SAFE_GET2(pThemeX->GetPrevNextEventControllerPartX(),
			...
			rcExclude = XTP_SAFE_GET2(pThemeX->GetPrevNextEventControllerPartX(),
...
void CXTPCalendarView::ShowToolTip(BOOL bShow)
{
	...
	if (GetCalendarControl()->IsToolTipVisible())
	{
		return;
	}
	...
	// Verify whether tooltip option is enabled
	BOOL bTipEnabled = XTP_SAFE_GET2(GetCalendarControl(), GetCalendarOptions(), bEnableAddNewTooltip, TRUE);
	if (!bTipEnabled)
		return;

	// create tooltip text
	CString strTip = XTP_SAFE_GET2(GetCalendarControl(), GetCalendarOptions(), strTooltipAddNewText, _T("Click to add appointment"));

	// show tooltip window
	CXTPCalendarControl* pCal = GetCalendarControl();
	if (pCal)
	{
		CFont* pFont = &XTP_SAFE_GET2(GetPaintManager(), GetDayViewEventPart(), GetTextFont(), *((CFont*)(NULL)));
		pCal->ShowToolTip(strTip, m_rcTooltip, pFont, TRUE);
	}
}
...
CString CXTPCalendarView::GetItemTextEventTimeMax()
{
