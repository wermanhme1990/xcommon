		...
		else if (hitInfo.uHitCode & xtpCalendarHitTestEventTextArea)
		{
				...
				!IsShowingToolTip() && !(nFlags&MK_LBUTTON) && m_bTextOutOfBorders)
			{
				if (m_nTimerID_ShowToolTip == 0)
				{
					...
					DBG_TRACE_TIMER(_T("SET Timer: ID =%d, m_nTimerID_ShowToolTip, CXTPCalendarViewEvent::OnMouseMove(xtpCalendarHitTestEvent_Mask) \n"), m_nTimerID_ShowToolTip);
				}
			}
		}
	}
...
CString CXTPCalendarViewEvent::GetToolText()
{
	...
	if (!IsMultidayEvent())
	{
		...
		strTime = GetItemText(dwTimeFlags);
	}
	...
	if (pControl)
	{
		if (pControl->GetPaintTheme() == xtpCalendarThemeOffice2007)
		{
			CString strLocation(m_ptrEvent->GetLocation());
...
CString CXTPCalendarViewEvent::GetItemText(DWORD dwItemFlag, LPCTSTR pcszDefaultText)
{
...
CString CXTPCalendarViewEvent::GetMarkupText()
{
...
//  /Calendar/Events/Event/EndTime
	...
	TRIMLEFT_S(strMarkupTemplate, _T(" \t\n\r"));
	TRIMRIGHT_S(strMarkupTemplate, _T(" \t\n\r"));

	if (strMarkupTemplate.Find(cszCalendarMarkupPrm_Subject) >= 0)
		REPLACE_S(strMarkupTemplate, cszCalendarMarkupPrm_Subject, GetItemTextSubject());

	if (strMarkupTemplate.Find(cszCalendarMarkupPrm_Location) >= 0)
		REPLACE_S(strMarkupTemplate, cszCalendarMarkupPrm_Location, GetItemTextLocation());

	if (strMarkupTemplate.Find(cszCalendarMarkupPrm_Body) >= 0)
		REPLACE_S(strMarkupTemplate, cszCalendarMarkupPrm_Body, GetItemTextBody());

	if (strMarkupTemplate.Find(cszCalendarMarkupPrm_StartTime) >= 0)
		REPLACE_S(strMarkupTemplate, cszCalendarMarkupPrm_StartTime, GetItemTextStartTime());

	if (strMarkupTemplate.Find(cszCalendarMarkupPrm_EndTime) >= 0)
		REPLACE_S(strMarkupTemplate, cszCalendarMarkupPrm_EndTime, GetItemTextEndTime());
