...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:key name="methods" match="methodsynopsis" use="methodname"/>
	<xsl:key name="methods_index" match="methodsynopsis" use="substring(methodname,1,1)"/>

	<xsl:template match="btdoc">
		<xsl:apply-templates select="classsynopsis"/>
		<p></p>
		<hr size="1" noshade="" align="center"></hr>
		<span class="link">NexgenIPL (c) 1999 - 2004 <a href="http://www.binary-technologies.com" target="_blank">Binary Technologies</a></span>
		<p></p>
	</xsl:template>
	...
	<xsl:template name="create-alphabeticalindex">
		<xsl:for-each select="methodsynopsis[count(. | key('methods_index', substring(methodname,1,1))[1]) = 1]">
			<xsl:sort select="methodname" data-type="text"/>
			<xsl:for-each select="key('methods_index', substring(methodname,1,1))">
				<xsl:sort select="methodname"/>
				<xsl:if test="position()=1">
					<a>
					<xsl:attribute name="href">#<xsl:value-of select="methodname"/></xsl:attribute>
					<xsl:value-of select="substring(methodname,1,1)"/>
					</a>
					<xsl:text>&#160;</xsl:text>
				</xsl:if>
			</xsl:for-each>
	...
	<xsl:template name="create-methodindex">
		...
		<a name="index"></a>
		<h2>Index</h2>
		<xsl:for-each select="methodsynopsis[count(. | key('methods', methodname)[1]) = 1]">
			<xsl:sort select="methodname" data-type="text"/>
			<a>
			<xsl:attribute name="href">#<xsl:value-of select="methodname"/></xsl:attribute>
			<xsl:value-of select="methodname"/>
